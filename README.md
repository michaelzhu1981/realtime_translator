...
一个自用开发版 macOS 菜单栏 App，用于捕获浏览器或播放器窗口音频，使用本地 MLX Whisper 做语音识别，再通过 LM Studio 的 OpenAI-compatible API 翻译，并在独立悬浮字幕窗中显示译文。

## 功能概览

- 菜单栏常驻 App，支持选择捕获窗口、开始/停止实时翻译。
- 使用 ScreenCaptureKit 捕获浏览器或播放器窗口音频。
- 通过常驻 Python ASR 服务调用 MLX Whisper，避免每个音频片段重复加载模型。
- 通过 LM Studio 本地模型接口进行翻译。
- 使用 SwiftUI + AppKit 显示透明置顶字幕窗。
- 支持输入语言、目标语言、字幕样式、ASR 模型缓存、超时和音频分片等基础设置。
- 菜单栏显示 ASR 原文、ASR 延迟和翻译延迟，方便调试。
...
- `ffmpeg`，ASR 服务启动前会检查 `/opt/homebrew/bin/ffmpeg`、`/usr/local/bin/ffmpeg` 或 `/usr/bin/ffmpeg`。
...
如果尚未安装 `ffmpeg`，可以使用 Homebrew 安装：
...
4. 运行 App，首次使用时按系统提示授予屏幕录制/系统音频录制相关权限。
5. 打开要翻译的浏览器或播放器窗口，在菜单栏里选择捕获窗口。
6. 点击“开始翻译所选窗口”。

ASR 模型默认使用 `mlx-community/whisper-large-v3-turbo`。模型默认缓存到 `~/Library/Caches/RealtimeTranslator/huggingface`，也可以在 App 设置中改为其他目录。

当前窗口列表优先显示常见浏览器和播放器，包括 Safari、Chrome、Firefox、Edge、Brave、Arc、Opera、Vivaldi、Chromium、QuickTime、VLC、IINA、mpv、Movist 和 Infuse。若目标窗口未出现在列表中，可以点击“显示全部可捕获窗口”后刷新。

## 工作机制

整体流程由 `TranslationPipeline` 串联：启动时先拉起 ASR 服务，再检查 LM Studio 连接，最后开始捕获所选窗口音频。运行中，ScreenCaptureKit 输出的音频样本会被写成短 WAV 分片，每个分片依次送入 ASR 服务转写；转写文本经过去重、稳定化和句子提交后，再发给 LM Studio 做流式翻译，翻译增量会实时更新到悬浮字幕窗。
...
音频捕获由 `WindowAudioCapture` 负责，使用 macOS `ScreenCaptureKit` 捕获所选窗口的系统音频，而不是麦克风输入。启动时会先检查屏幕录制/系统音频录制权限，然后确认目标窗口仍可捕获；如果窗口关闭、不可见或权限未授权，App 会直接给出错误提示。
...
App 还会监听目标窗口位置变化，每约 250 ms 刷新一次窗口 frame，用于让字幕窗跟随播放器窗口。若捕获启动后 5 秒内没有收到音频样本，会提示用户检查目标窗口是否正在播放有声音的视频，以及系统权限是否完整。
...
- ASR 模型缓存：默认 `~/Library/Caches/RealtimeTranslator/huggingface`。
- 音频分片缓存：`~/Library/Caches/RealtimeTranslator/audio-chunks`。
- App 日志：`~/Library/Logs/RealtimeTranslator/app.log`。
- 用户设置：通过 `UserDefaults` 保存。

## 常见问题

- 如果没有可选窗口，先确认目标浏览器或播放器窗口在屏幕上可见，然后点击菜单栏里的“刷新”。必要时打开“显示全部可捕获窗口”。
- 如果提示没有屏幕录制权限，请在系统设置的“隐私与安全性 > 屏幕与系统音频录制”中允许 RealtimeTranslator，然后重启 App。
- 如果 ASR 启动失败，优先检查 ASR Python Path、虚拟环境里的 `mlx-whisper`、`ffmpeg` 和模型缓存目录是否可写。
- 如果无法连接 LM Studio，请确认 LM Studio server 已开启、Base URL 可访问、模型名匹配，并在系统“本地网络”权限中允许 App 访问局域网服务。
- 如果已开始捕获但没有字幕，确认目标窗口正在播放有声音的视频；某些 DRM 内容或无系统音频输出的场景可能无法被 ScreenCaptureKit 正常捕获。
